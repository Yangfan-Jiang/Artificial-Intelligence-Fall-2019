...
//		thisChoice.score = pNextChoice->score;
		...
		// The original code is written in Negamax style
		// ineach layer, max min exchange with different sign, as a result, 
		// the number of beta layer are all negative.
		// To decide whether or not to prune (alpha >= beta),
		// We just have to figure out if score >= max at each level is true
		
		// Minimax style AlphaBeta prune algorithm 
		// beta layer
		if (player == -1) {
			int alpha = -max;
			int beta = -min;
